...
- **默认无感登录**：用户进入小程序即自动创建/恢复账号（基于微信云开发 OPENID），不弹授权框。
- **资料授权（可选）**：在个人中心点击“微信授权”后，通过 `getUserProfile` 获取昵称/头像用于展示与个性化。
- **绑定手机号（可选）**：在个人中心点击“绑定手机号”后，获取并绑定手机号，用于后续在 App 端支持“手机号登录/微信登录”与同一账号记录关联。
- **隐私约束**：前端本地仅缓存必要字段（用户ID、昵称/头像、是否已绑定手机号），不缓存手机号明文。
...
#### 2.2.2 愿望输入
- **功能**：输入最近许过但没成功的愿望
- **要求**：默认无感登录即可使用；昵称/头像授权为可选
- **操作**：点击"开始分析"按钮

#### 2.2.3 分析结果
- **缺失要素**：时间边界、量化目标、行动承诺等
- **潜在原因/后果**：愿望过于抽象、目标不可验证等
- **建议许愿对象**：由 AI 结合愿望内容与用户目标动态推荐更契合的对象（如学业→文殊、姻缘→月老、犯太岁→太岁）
- **正确姿势**：锁定状态，需要解锁
...
### 3.2 安全要求
- API Key 保护：大模型 API Key 必须通过云函数环境变量配置（优先 Qwen-Flash，备选 Kimi，最后 DeepSeek）
- 内容安全：必须调用微信内容安全 API
...
- **云数据库**：微信云数据库
- **外部服务**：大模型 API（优先 Qwen-Flash，备选 Kimi，最后 DeepSeek）
...
### Phase 1：核心功能（MVP）
1. ✅ 用户登录（微信授权）
2. ✅ 愿望分析接口（调用大模型：优先 Qwen-Flash，备选 Kimi，最后 DeepSeek）
3. ✅ TODO 列表 CRUD
4. ✅ 基础内容安全过滤
